...
module.exports = function(transforms, contents, cb) {
  ...
  var visitorFactory = function() {
    ...
    ret.visit = function(node, parent, context) {
      console.log('Node Expression object: ', get(node, 'expression'));
      console.log('Node Expression.type: ',get(node, 'expression.type'));
      console.log('Node Expression.callee.name: ', get(node, 'expression.callee.name'));
      console.log('Node Expression.arguments: ', get(node, 'expression.arguments'));
      if (node.type === 'ExpressionStatement' &&
          get(node, 'expression.type') === 'CallExpression' &&
          (get(node, 'expression.callee.name') === 'describe' || get(node, 'expression.callee.property['Identifier'].name') === 'describe' || get(node, 'expression.callee.name') === 'context') &&
